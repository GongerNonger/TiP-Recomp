...
public:
    std::string name;
    void Tick();
    int AverageCount = 500; // Number of frames to average over
    ...
    void OnDraw(ImGuiIO& io) override {
        ...
        if(ShowPlot) {
            if (ImPlot::BeginPlot("FPS Plot", ImVec2(-1, 0))) {
                ImPlot::SetupAxesLimits(0, 500, 0, 200, ImPlotCond_Always);
                for (auto& counter : fpsManager->counters) {
                    if (counter->frameTimes.size() >= 2) {
                        int count = static_cast<int>(counter->frameTimes.size());
                        int offset = count > 500 ? count - 500 : 0;
                        int plotCount = count > 500 ? 500 : count;
