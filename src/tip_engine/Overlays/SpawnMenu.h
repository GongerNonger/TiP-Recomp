...
struct SpawnRequest {
    ...
    bool wildcardAsEgg = true; // true = spawn egg for full wildcard, false = spawn adult (model only)
};
...
inline bool g_CursorTargetPending = false; // unused (crashes)
// Find entity by tag — processed in gardenMainGetGardenScene hook
struct FindEntityRequest {
    bool pending = false;
    uint32_t tagID = 0;
};
inline FindEntityRequest g_FindEntity;
...
// Native PV barcode injection — feeds raw barcode through game's own decoder
struct NativePVInject {
    bool pending = false;
    std::string hexRow1;  // 16-char hex for barcode row 1
    std::string hexRow2;  // 16-char hex for barcode row 2 (optional)
};
inline NativePVInject g_NativePVInject;

// Model dump request
inline bool g_ModelDumpRequested = false;

// Deferred wildcard application (entity needs time to initialize)
struct DeferredWildcard {
    uint32_t entity = 0;
    uint32_t tagID = 0;
    int wildcardTrait = 0;
    int framesRemaining = 0;
};
inline DeferredWildcard g_DeferredWildcard;

// Wildcard breeding override — forces all hatched eggs to be wildcards
